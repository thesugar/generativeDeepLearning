...
class VAEModel(Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # 復元損失（reconstruction loss）: 普通の RMSE と同じ
            reconstruction = self.decoder(z)
            reconstruction_loss = tf.reduce_mean(
                tf.square(data - reconstruction), axis=[1,2,3]
            )
            reconstruction_loss *= self.r_loss_factor  # r_loss_factor: 復元損失と KLD のバランスを取るための重みづけ（p.94）
            # KLD = -(1/2) * Σ(1 + log(σ^2) - μ^2 - σ^2)
    ...
    def call(self, inputs):
        z_mean, z_log_var, latent = self.encoder(inputs)  ## https://github.com/davidADSP/GDL_code/issues/83#issuecomment-748477879
...
class VariationalAutoencoder():
    def __init__(self
        ...
        self.name = 'variational__autoencoder'
    ...
    def _build(self):
        ...
        self.z = Sampling(name='encoder_output')([self.mu, self.log_var])  # mu（μ）と log_var を使って z を求める（Sampling クラスの実装を参照）。z = μ + σ * ε
    ...
    def train(self, x_train, batch_size, epochs, run_folder, print_every_n_batches = 100, initial_epoch = 0, lr_decay = 1):
        ...
        checkpoint_filepath=os.path.join(run_folder, "weights/weights.h5")  # https://github.com/davidADSP/GDL_code/issues/73#issuecomment-725401490
